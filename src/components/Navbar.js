import { BsFillGrid1X2Fill, BsStickiesFill, BsFillPeopleFill, BsPlusCircle, BsBrightnessHigh } from "react-icons/bs";
    ...
    <nav className="z-40 flex fixed sm:static sm:block bottom-0 w-screen select-none sm:w-[260px] sm:min-h-screen sm:max-h-full bg-primary-theme">
      <div className="wrapper h-full w-10/12 sm:w-4/5 mx-auto py-4 sm:py-8">
        <ul className="flex justify-between sm:gap-12 sm:flex-col sm:h-full">
          <Link className="hidden sm:block" to="/orders">
            <h1 className=" text-primary-color text-3xl font-semibold mb-12">Eligöten.</h1>
          </Link>
          <NavLink
            className={(navData) =>
              navData.isActive
            ...
            to="/orders"
          >
            <BsFillGrid1X2Fill className="w-[32px] h-[32px] sm:w-[24px] sm:h-[24px]" /> <p className="hidden sm:block">Orders</p>
          </NavLink>
          <NavLink
            className={(navData) =>
              navData.isActive
